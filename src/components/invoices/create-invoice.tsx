...
import React, { Dispatch, FormEvent, MutableRefObject, SetStateAction, useEffect, useMemo, useState } from 'react'
            ...
            <div className='w-full h-full flex justify-center md:items-center'>
                <div className='relative max-w-[400px] h-auto overflow-y-auto no-scrollbar w-full gap-2 text-sm flex flex-col rounded bg-white p-4'>
                    ...
                    <form ref={formRef} onSubmit={(e) => handleSubmit(e)} className='w-full flex flex-col gap-1'>
                        ...
                        <Input required
                            title='Dealer Code'
                            type='number' name='dealercode'
                            placeholder='240000' />
                        <Input title='Name' readOnly
                            type='text' name='name'
                            placeholder='01FF' />
                        ...
                        <div className='text-xs flex flex-col gap-1'>
                            {list.length > 0 && list.map((ld, i) =>
                                <div onClick={() => setList(list.filter(item => item.productid !== ld.productid))} key={i} className='flex items-center'>
                                    ...
                                    <span><XMarkIcon className='w-4' /></span>
                                    <span>{ld.quantity}</span>
                                </div>
                            )}
                        </div>
                        <Input dval={mrpbill} readOnly title='MRP' type='number' name='mrp' placeholder='0.00' />
                        <Input title='Extra Discount' type='number' name='exdiscount' placeholder='0%' />
                        <Input title='Taxs' type='number' name='exdiscount' placeholder='0%' />
                        <div className='flex mb-2 gap-1 w-full items-end'>
                            <Input readOnly title='Total Bill' type='number' name='tbill' placeholder='0%' />
                            <Button type='button'
                                disabled={isLoading}
